...
use crate::gossip::{ NodeMemory, HLC };
use crate::gossip;
use uuid::Uuid;
...
fn with_socket(socket: Arc<UdpSocket>) -> impl Filter<Extract = (Arc<UdpSocket>,), Error = std::convert::Infallible> + Clone {
...
pub async fn web_server(local_web_addr: String, socket: Arc<UdpSocket>, memory: Arc<Mutex<NodeMemory>>) {
    ...
    warp::serve(post_item.or(get_item).or(get_items_count).or(remove_item)).run(address).await;
}