use crate::web;
use crate::gossip::{receive_gossip, send_gossip, send_gossip_single, NodeMemory};
use log::info;
use names::Generator;
...
pub async fn start_node(web_addr: String, local_addr: String, node_memory: Arc<Mutex<NodeMemory>>) {
    ...
    let _ = tokio::spawn(send_gossip(node_name.clone(), local_addr.clone(), socket.clone(), node_memory.clone(), sync_flag.clone()));
    let _ = tokio::spawn(receive_gossip(node_name.clone(), socket.clone(), node_memory.clone(), sync_flag.clone()));
